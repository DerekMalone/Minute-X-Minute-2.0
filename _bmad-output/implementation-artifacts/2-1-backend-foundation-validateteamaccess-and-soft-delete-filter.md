...
- `backend/Migrations/` — new (generated by EF Core)
...
**Issues Fixed (5):**
- [H1] `JoinedAt` not auto-tracked — added `"JoinedAt"` to `SetTimestamps()` for `Added` entities (`AppDbContext.cs`)
- [M1] `MemberRole` enum stored as integer with no locked values — added explicit `= 0`, `= 1` assignments (`MemberRole.cs`)
- [M2] `SaveChanges()` sync override missing — extracted `SetTimestamps()` private method; added sync override (`AppDbContext.cs`)
- [M3] Drill → Team FK used `CASCADE` delete, bypassing soft-delete strategy — changed to `DeleteBehavior.Restrict` in `OnModelCreating` (`AppDbContext.cs`). **Requires new migration — run:**
  ```bash
  dotnet ef migrations add FixDrillFKCascade --project backend
  dotnet ef database update --project backend
  ```
- [M4] Story File List documented migration path as `backend/Data/Migrations/` — corrected to `backend/Migrations/`

**Issues Deferred (3 Low — not blocking):**
- [L1] Dev Notes says "No Program.cs Changes Required" but Program.cs was modified — documentation inconsistency
- [L2] `SetTimestamps()` uses magic strings for property matching — no compile-time safety
- [L3] No global exception handler for `UnauthorizedAccessException → 403` — per-controller catch blocks required in all future stories
